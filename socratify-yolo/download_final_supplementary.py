#!/usr/bin/env python3
"""
Download final supplementary logos - The absolute final piece
304 companies to make this truly comprehensive
"""
...
import time
from final_supplementary_companies import get_all_supplementary_companies
from final_supplementary_companies import (
    SOUTHEAST_ASIA_COMPANIES, MIDDLE_EAST_COMPANIES, SHIPPING_PORTS,
    ENGINEERING_CONSTRUCTION, CHEMICALS_MATERIALS, TOBACCO_ALCOHOL,
    PROFESSIONAL_SERVICES, TRADITIONAL_INDUSTRIES, EMERGING_CATEGORIES,
    REGIONAL_SPECIALISTS
)

# Create output directory
OUTPUT_DIR = 'logos/final_supplementary'
...
def company_already_exists(company_name):
    ...
    for part in parts:
        if len(part) > 4 and part in existing_logos:
            ...
            if part in ['bank', 'group', 'capital', 'digital', 'global', 'international', 'holding', 'airlines']:
                continue
            return True
...
def download_company_logo(company_name, domain):
    ...
    # For .ae, .sa, .qa domains try .com
    if domain.endswith(('.ae', '.sa', '.qa', '.tr', '.id', '.th', '.vn', '.my', '.ph')):
        base = domain.split('.')[0]
...
def main():
    all_companies = get_all_supplementary_companies()
    
    print("=" * 80)
    print("FINAL SUPPLEMENTARY DOWNLOAD - THE ABSOLUTE FINAL PIECE")
    print("=" * 80)
    ...
    # Categories
    categories = [
        ("SOUTHEAST ASIA", SOUTHEAST_ASIA_COMPANIES),
        ("MIDDLE EAST", MIDDLE_EAST_COMPANIES),
        ("SHIPPING & PORTS", SHIPPING_PORTS),
        ("ENGINEERING & CONSTRUCTION", ENGINEERING_CONSTRUCTION),
        ("CHEMICALS & MATERIALS", CHEMICALS_MATERIALS),
        ("TOBACCO & ALCOHOL", TOBACCO_ALCOHOL),
        ("PROFESSIONAL SERVICES", PROFESSIONAL_SERVICES),
        ("TRADITIONAL INDUSTRIES", TRADITIONAL_INDUSTRIES),
        ("EMERGING CATEGORIES", EMERGING_CATEGORIES),
        ("REGIONAL SPECIALISTS", REGIONAL_SPECIALISTS)
    ]
    ...
    for category_name, companies in categories:
        ...
        # Save progress
        with open('logos/final_supplementary_results.json', 'w') as f:
            json.dump(all_results, f, indent=2)
    
    # Final summary
    print("\n" + "=" * 80)
    print("FINAL SUPPLEMENTARY COMPLETE")
    print("=" * 80)
    ...
    print(f"\nResults: logos/final_supplementary_results.json")
