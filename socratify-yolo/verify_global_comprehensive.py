#!/usr/bin/env python3
"""
Verify comprehensive global missing list against existing collection
Check which of the 358 companies we actually don't have
"""

import os
import re
from global_comprehensive_missing import get_comprehensive_global_missing
        ...
        if os.path.exists(base_dir):
            for file in os.listdir(base_dir):
                if file.endswith(('.png', '.jpg', '.jpeg', '.svg', '.ico', '.webp')):
                    ...
                    clean_name = re.sub(r'(_\d+|_logo|_icon|_favicon|_inc|_corp|_corporation|_company|_group|_ltd|_llc|_sa|_ag|_se|_plc|_holdings)$', '', name.lower())
...
def main():
                ...
                if len(normalized) > 2 and len(exist) > 2:
                    if normalized in exist or exist in normalized:
                        ...
                        if len(normalized) >= 3 and len(exist) >= 3:
                            ...
                            if match_ratio > 0.7:  # At least 70% overlap for global companies
    ...
    # Save results
    with open('/Users/adi/code/socratify/socratify-yolo/global_verified_missing.txt', 'w') as f:
        f.write("GLOBAL VERIFIED MISSING COMPANIES FOR DOWNLOAD\n")
        f.write("=" * 55 + "\n\n")
    ...
    print(f"\nSaved verification results to global_verified_missing.txt")
    print(f"\nTop 30 actually missing companies:")
