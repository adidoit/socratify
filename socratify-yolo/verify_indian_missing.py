#!/usr/bin/env python3
"""
Verify comprehensive Indian companies against existing collection
Check which of the 216 companies we actually don't have
"""

import os
import re
from comprehensive_indian_companies import get_comprehensive_indian_companies
        ...
        if os.path.exists(base_dir):
            for file in os.listdir(base_dir):
                if file.endswith(('.png', '.jpg', '.jpeg', '.svg', '.ico', '.webp')):
                    ...
                    clean_name = re.sub(r'(_\d+|_logo|_icon|_favicon|_inc|_corp|_corporation|_company|_group|_ltd|_llc|_sa|_ag|_se|_plc|_holdings|_limited)$', '', name.lower())
...
def main():
    ...
    print(f"Indian companies to check: {len(unique_companies)}")
                ...
                if len(normalized) > 2 and len(exist) > 2:
                    if normalized in exist or exist in normalized:
                        ...
                        if len(normalized) >= 3 and len(exist) >= 3:
                            ...
                            if match_ratio > 0.6:  # At least 60% overlap for Indian companies
    ...
    # Save results
    with open('/Users/adi/code/socratify/socratify-yolo/indian_verified_missing.txt', 'w') as f:
        f.write("VERIFIED MISSING INDIAN COMPANIES FOR DOWNLOAD\n")
        f.write("=" * 50 + "\n\n")
        ...
        f.write("\n" + "=" * 50 + "\n")
        f.write("ALL MISSING INDIAN COMPANIES (ALPHABETICAL):\n")
        f.write("=" * 40 + "\n\n")
        
        for i, company in enumerate(sorted(actually_missing), 1):
            f.write(f"{i:3d}. {company}\n")
        
        f.write(f"\n\n=== SAMPLE OF ALREADY HAVE ({len(already_have)} companies) ===\n")
        for company in already_have[:30]:  # Show first 30
            f.write(f"✅ {company}\n")
        if len(already_have) > 30:
            f.write(f"... and {len(already_have) - 30} more\n")
    
    print(f"\nSaved verification results to indian_verified_missing.txt")
    print(f"\nTop 30 actually missing Indian companies:")
    for i, company in enumerate(sorted(actually_missing)[:30], 1):
        print(f"{i:2d}. {company}")
    
    print(f"\nCategories with most missing companies:")
    for category, companies in sorted(missing_by_category.items(), key=lambda x: len(x[1]), reverse=True)[:10]:
